...
from juggertube.api.serializing import serialize_team, serialize_user
...
def register():
    ...
    new_user = User(username=post_data["username"], email=post_data["email"], password_hash=post_data["password_hash"],
                    team=post_data["team"])
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, post_data["password_hash"]):
            login_user(user)
